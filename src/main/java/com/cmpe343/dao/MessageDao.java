...
public class MessageDao {
        ...
        try (Connection c = Db.getConnection();
            ...
            while (rs.next()) {
                ...
                list.add(new Message(
                        rs.getInt("id"),
                        -1, // Sender ID unknown in this query for now or we can join
                        rs.getString("sender"),
        ...
        try (Connection c = Db.getConnection();
            ...
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
                    ...
                    list.add(new Message(
                            rs.getInt("id"),
                            -1, // We could join but for now just fix constructor
                            rs.getString("sender"),
                            rs.getString("content"),
                            messageTime,
                            rs.getBoolean("is_read")));
     ...
     * @return List of messages in chronological order (or reverse desc)
     */
    public List<Message> getMessagesBetween(int ownerId, int otherUserId) {
        List<Message> list = new ArrayList<>();
        // Removed owner_id filter to show all messages from this customer
        String sql = """
                    SELECT m.id, u.username as sender, m.text_clob as content,
                           m.created_at, (m.replied_at IS NOT NULL) as is_read,
        ...
        try (Connection c = Db.getConnection();
            ...
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
                    ...
                    // Logic:
                    // If sender_id is present, use it to determine if it is OWNER.
                    // If sender_id is NULL (legacy), assume Customer sent it if reply_text is
                    // empty,
                    // or handle reply as separate message?
                    // Actually, for legacy rows (customer msg + optional owner reply), we can keep
                    // showing them as bubbles.
                    // But for NEW rows (one msg per row), sender_id tells us who sent it.

                    int senderId = rs.getInt("sender_id");
                    boolean hasSenderId = !rs.wasNull();

                    // Case 1: The row is a message (either from customer or owner)
                    // If hasSenderId, use it. If not, assume it's from Customer (legacy).
                    boolean isFromOwner = hasSenderId && (senderId == ownerId);

                    // Legacy fallback: if senderId is 0 (null in DB), it is likely customer
                    if (!hasSenderId) {
                    ...
                    Message msg = new Message(
                            rs.getInt("id"),
                            senderId,
                            isFromOwner ? "Owner" : rs.getString("sender"),
                            rs.getString("content"),
                            messageTime,
                            rs.getBoolean("is_read"));
                    ...
                    // Case 2: Legacy Reply (reply_text exists)
                    String reply = rs.getString("reply_text");
                    if (reply != null && !reply.isEmpty()) {
                        Message replyMsg = new Message(
                                -rs.getInt("id"),
                                ownerId, // Owner sent this
                                "Owner",
                                reply,
                                rs.getTimestamp("replied_at") != null ? rs.getTimestamp("replied_at").toLocalDateTime()
